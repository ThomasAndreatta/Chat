...
# region COSE DA FARE
        ...
        //cripto alla chat con chiave inserita nel form iniziale
        //invio del numero degli utenti quando qualcuno logga e salvare il numero maggiore, mandare anche lo struct "numero|nome|ip" e aggiornare in base all'ultimo
        #endregion
        ...
        private void Ascolta()
        {
            ...
            while (continua)
            {
                ...
                if (msg.Contains("ENTRA NELLA CHAT"))
                {
                 ...
                 else if (msg.Contains("ESCE DALLA CHAT"))
                {
                    lstBoxMsg.Items.Add(msg.Split('|')[2]);
                    listaUtenti.nomi.RemoveAll((x) => x == msg.Split('|')[1]);
                    listaUtenti.ip.RemoveAll((x) => x == IPAddress.Parse(msg.Split('|')[0]));
                    CountPersone--;
                    lblPersone.Text = "" + CountPersone;
                }else
                {
